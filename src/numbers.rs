...
fn parse_without_dot(number_capture: Option<Match>, factor: usize, deviation_factor: f64) -> Option<usize> {
    let num_str = number_capture.unwrap().as_str();
    let num = num_str.parse::<usize>().unwrap();
    let generated_number = randomize_around(factor, deviation_factor);
...
fn parse_with_dot(
    ...
    let generated_number = randomize_around(existing_numbers_factor, deviation_factor);

    return Some((number_before_randomizing as isize + generated_number) as usize);
}

fn randomize_around(value: usize, deviation_factor: f64) -> isize {
    let difference = (value as f64 * deviation_factor) as isize;
